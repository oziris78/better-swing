package com.telek.betterswing.image;


import com.telek.betterswing.utils.BetterSwingException.*;
...
public final class BufferedImgUtils {

	/* No Constructor */
	private BufferedImgUtils(){}
	...
	public static BufferedImage getBlurredImage(BufferedImage imageToBlur, int neighborCount, double similarity){
		if(similarity > 1 || similarity < 0)
			throw new ValueOutOfRangeException(similarity, 0, 1);
	...
	public static BufferedImage getGrayscaledImage(BufferedImage imageToChange, int harshness, boolean setColorToDarkAreas, Color secondaryColor){
		if(harshness >= 256 || harshness < 0)
			throw new ValueOutOfRangeException(harshness, 0, 255);
		BufferedImage img = getCopyOf(imageToChange);
		Color currentColor;
		Color newColor;
		for(int i = 0; i < img.getWidth(); i++){
